...
public class SpellsAlien : NetworkBehaviour
{
    ...
    void Start()
    {
		...
		// NEED TO FIND THE RELEVANT PLAYER SHOOTING POINT

		//frontPos = transform.Find("ShootingPoint");
    ...
    private void RemainingTime() // Calculate the remaining time before next use, which will be used for the UI
    {
        if (remainingTime1 > 0)
        {
            ...
            spell1T.GetComponent<Text>().text = remainingTime1.ToString();
            if (remainingTime1 < 0)
            {
                remainingTime1 = 0;
        ...
        if (remainingTime2 > 0)
        {
            ...
            spell2T.GetComponent<Text>().text = remainingTime2.ToString();
            if (remainingTime2 < 0)
            {
                remainingTime2 = 0;
